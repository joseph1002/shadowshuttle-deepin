...
void
ShadowsocksController::saveServers(const QList<ServerConfig> &servers, int localPort, int index, bool portableMode) {
...
QString ShadowsocksController::getLocalPacUrl() {
    return pacServer->getLocalPacUrl(configuration);
}

QString ShadowsocksController::touchUserRuleFile() {
    return pacServer->touchUserRuleFile();
}
